...
class PlaylistService {
   ...
   */
   async updatePlaylist (playlist) {
    ...
    await this.savePlaylistThumbnail(playlist);
    const theFilter = { id: playlist.id };
    const setQuery = {
      $set: {
    ...
    await this.collection.updateOne(theFilter, setQuery);
  }
  ...
  async search (substring, exact) {
    ...
    if (exact) {
      ...
      const filterDescription = { description: { $regex: `${substring}` } };
      playlists = await this.collection.find({ $or: [filterName, filterDescription] }).toArray();
    } else {
